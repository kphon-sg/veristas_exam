...
export const StudentScoresView: React.FC<StudentScoresViewProps> = ({ teacherId, onClose }) => {
  ...
  useEffect(() => {
    ...
    if (viewState === 'STUDENTS' && selectedClass) {
      ...
      fetch(`/api/teacher/courses/${selectedClass.id}/students`)
        .then(async res => {
          if (!res.ok) {
            const errData = await res.json().catch(() => ({}));
            throw new Error(errData.error || `Server error: ${res.status}`);
  ...
  const fetchStudentQuizzes = useCallback(() => {
    if (viewState === 'QUIZZES' && selectedClass && selectedStudent) {
      ...
      fetch(`/api/teacher/courses/${selectedClass.id}/students/${selectedStudent.id}/quizzes`)
        .then(async res => {
          if (!res.ok) {
            const errData = await res.json().catch(() => ({}));
            throw new Error(errData.error || `Server error: ${res.status}`);
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-[#f4f7f9] flex flex-col">
      {/* Header */}
      <div className="h-16 border-b border-portal-300 bg-portal-500 flex items-center justify-between px-6 sticky top-0 z-50 shadow-md">
        <div className="flex items-center gap-6">
          <button 
            onClick={handleBack}
            className="w-10 h-10 bg-white/10 border border-white/20 rounded-full flex items-center justify-center hover:bg-white/20 transition-colors shadow-sm"
          >
          ...
          <div>
            <h2 className="text-xl font-black text-white tracking-tight">
              ...
              {viewState === 'STUDENTS' && `Class: ${selectedClass?.courseCode} - ${selectedClass?.name}`}
              {viewState === 'QUIZZES' && `Student: ${selectedStudent?.fullName}`}
            </h2>
            <div className="flex items-center gap-2">
              <span className="text-[10px] font-mono uppercase tracking-widest text-portal-100 font-bold">
                {viewState === 'CLASSES' && "Select a class to view students"}
                {viewState === 'STUDENTS' && `${students.length} Students Enrolled`}
        ...
        <button 
          onClick={onClose}
          className="px-4 py-2 bg-white/10 border border-white/20 rounded-md text-xs font-bold text-white hover:bg-white/20 transition-all"
        >
          Close View
        </button>
      </div>
