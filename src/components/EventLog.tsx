  ...
  return (
    <div className="flex flex-col h-full bg-white border border-portal-200 rounded-lg overflow-hidden shadow-sm">
      <div className="px-4 py-2 border-b border-portal-100 bg-portal-50 flex items-center justify-between">
        <h3 className="text-[10px] font-mono uppercase tracking-widest text-portal-600 flex items-center gap-2 font-black">
          <ShieldAlert className="w-3.5 h-3.5 text-amber-500" />
          Behavior Log
        </h3>
        <span className="text-[10px] font-mono text-portal-400 uppercase tracking-tighter font-bold">{events.length} Events</span>
      </div>
            ...
            events.map((event) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[11px] text-slate-500 leading-tight font-medium">{event.message}</p>
                </div>
              </motion.div>
            ))
          )}
