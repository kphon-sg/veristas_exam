...
export const SubmissionReview: React.FC<SubmissionReviewProps> = ({ quiz: initialQuiz, onClose, initialSubmissionId, studentId }) => {
  ...
  const handleSaveGrades = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert("Grades saved successfully! This submission is now finalized.");
      } else {
        const errData = await res.json();
        console.error("[SubmissionReview] Failed to save grades:", errData);
        alert(`Failed to save grades: ${errData.error || 'Unknown error'}`);
      }
    } catch (err) {
      console.error("[SubmissionReview] Network error while saving grades:", err);
      alert("Network error while saving grades.");
    } finally {
      setSaving(false);
    }
  };
  ...
  const updatePoints = (questionId: number, points: number) => {
    ...
    const newAnswers = selectedSubmission.answers.map((a: any) => 
      a.questionId === questionId ? { ...a, pointsEarned: points } : a
    );
    const newScore = newAnswers.reduce((acc: number, a: any) => acc + (a.pointsEarned || 0), 0);
    setSelectedSubmission({ ...selectedSubmission, answers: newAnswers, score: newScore });
  };

  const totalPossiblePoints = (quiz.questions || []).reduce((acc: number, q: any) => acc + (q.points || 1), 0);
      ...
      >
        ...
        <div className="portal-card-header flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-xl font-black text-white">Submissions: {quiz.title}</h2>
              <p className="text-xs text-portal-100 font-mono uppercase tracking-widest">{submissions.length} students submitted</p>
            </div>
          </div>
                      ...
                      <div className="flex flex-wrap items-center gap-4 mt-3">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xl font-black text-portal-600 font-mono">
                            {Number(selectedSubmission.score || 0).toFixed(1)} <span className="text-slate-400 text-sm font-normal">/ {Number(selectedSubmission.totalScore || 0).toFixed(1)}</span>
                          </span>
                        </div>
