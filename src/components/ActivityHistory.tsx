...
export const ActivityHistory: React.FC<ActivityHistoryProps> = ({ userId, role, onClose }) => {
  ...
  const getIcon = (type: string) => {
    ...
    switch (actionType) {
      ...
      case 'JOIN_REQUEST_ACCEPTED': return <CheckCircle2 className="w-4 h-4 text-emerald-500" />;
      case 'JOIN_REQUEST_DECLINED': return <XCircle className="w-4 h-4 text-rose-500" />;
    ...
    <div className="flex flex-col h-full bg-white rounded-2xl shadow-xl overflow-hidden border border-slate-100">
      ...
      <div className="p-6 bg-slate-50 border-b border-slate-100 flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-slate-400 font-mono mt-1 uppercase tracking-widest">Audit log of all your actions</p>
        </div>
        {onClose && (
