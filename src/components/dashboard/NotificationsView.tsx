...
interface Notification {
  id: number;
  type: 'INVITATION' | 'JOIN_REQUEST' | 'SYSTEM' | 'INVITATION_RESPONSE' | 'JOIN_RESPONSE' | 'NEW_QUIZ' | 'QUIZ_GRADED';
  title: string;
  message: string;
  relatedId: number;
  isRead: boolean;
                  ...
                  {otherNotifications.map((notif, idx) => (
                    ...
                    >
                      <div className={cn(
                        ...
                        notif.isRead ? "bg-slate-100 text-slate-400" : (
                          notif.type === 'NEW_QUIZ' ? "bg-amber-100 text-amber-600" :
                          notif.type === 'QUIZ_GRADED' ? "bg-emerald-100 text-emerald-600" :
                          "bg-violet-100 text-violet-600"
                        )
                      )}>
                        {notif.type.includes('RESPONSE') ? <Check className="w-5 h-5" /> : 
                         notif.type === 'NEW_QUIZ' ? <BookOpen className="w-5 h-5" /> :
                         notif.type === 'QUIZ_GRADED' ? <GraduationCap className="w-5 h-5" /> :
