      ...
      if (status === 'PUBLISHED') {
        ...
        for (const student of enrolledStudents) {
          await db.query(
            ...
            [student.student_id, 'NEW_QUIZ', 'New Quiz', `${courseInfo} has been published.`, quizId]
          );
        }
      }
  ...
  try {
    ...
    if (quiz) {
      ...
      if (status === 'PUBLISHED') {
        ...
        for (const student of enrolledStudents) {
          // Check if notification already exists to avoid duplicates on every update
          const [existingNotif]: any = await db.query(
            "SELECT id FROM notifications WHERE user_id = ? AND type = 'NEW_QUIZ' AND related_id = ?",
          ...
          if (existingNotif.length === 0) {
            await db.query(
              "INSERT INTO notifications (user_id, type, title, message, related_id) VALUES (?, ?, ?, ?, ?)",
              [student.student_id, 'NEW_QUIZ', 'New Quiz', `${quiz.course_code} - ${title} has been published.`, quizId]
            );
          }
        }
      }
